...
export default function SinglePostPage() {
  ...
  // 🚀 લિંક ભલે ગમે તે હોય, આપણે ડિફોલ્ટ ગુજરાતી જ રાખ્યું છે
    ...
    const fetchPost = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          // 🚀 વાંચવાનો સમય ગણવા માટે સીધું ગુજરાતી કન્ટેન્ટ જ પકડશે!
          const text = data.translations?.['gu']?.content || data.translations?.['en']?.content || '';
  ...
  // 🚀 અસલી માસ્ટરસ્ટ્રોક: હવે એ સીધું ગુજરાતી જ ગોતશે!
  const getTrans = (field: string) => {
    const guVal = post.translations?.['gu']?.[field];
    if (guVal && guVal.trim() !== '') return guVal;
    // જો કોઈ કારણસર ગુજરાતી ના મળે, તો જ બીજું શોધશે
    return post.translations?.['en']?.[field] || post.translations?.['hi']?.[field] || '';
  };
  ...
  // 🚀 તારીખ પણ હવે મસ્ત રીતે દેખાશે
  const postDate = post.createdAt ? new Date(post.createdAt.toMillis()).toLocaleDateString('gu-IN', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'Just Now';
    ...
    <div className="bg-white min-h-screen pb-20 font-sans selection:bg-blue-200 selection:text-blue-900">
      ...
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-8">
          ...
          {/* 🚀 અસલી ન્યૂઝનું લખાણ અહીં દેખાશે! */}
          <div 
