...
export default function HomePage() {
  ...
  // ટાઈટલ અને ડિસ્ક્રિપ્શન લાવવા માટે
  ...
  useEffect(() => {
    const fetchHomeData = async () => {
      try {
        ...
        // 🚀 ફાઇનલ ફિલ્ટર: ખાલી એ જ ન્યૂઝ દેખાશે જે 'published' છે અને જેમાં 'ફોટો' (Image) છે!
        const validNews = rawDocs.filter((news: any) => {
          const isPublished = (news.status || '').toLowerCase() === 'published';
          const hasImage = news.featuredImage && news.featuredImage.trim() !== '';
          return isPublished && hasImage;
        });

        validNews.sort((a: any, b: any) => {
          const timeA = a.createdAt?.toMillis ? a.createdAt.toMillis() : 0;
          const timeB = b.createdAt?.toMillis ? b.createdAt.toMillis() : 0;
          return timeB - timeA;
        });

        setFeaturedNews(validNews.filter((n: any) => n.placement?.isFeatured));
        setTrendingNews(validNews.filter((n: any) => n.placement?.isTrending).slice(0, 5));
        setHomeNews(validNews); 
            ...
            <Link href={`/${lang}/post/${featuredNews[0].id}`}>
              ...
              <div className="absolute bottom-0 left-0 p-6 md:p-8 w-full">
                <span className="bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-md uppercase tracking-wider mb-4 inline-block">
                  {featuredNews[0].category || 'BREAKING'}
                </span>
                <h1 className="text-2xl md:text-4xl font-black text-white leading-tight drop-shadow-lg">
                  {getTitle(featuredNews[0])}
                </h1>
                <p className="text-slate-300 mt-3 text-base md:text-lg line-clamp-2 hidden md:block">
